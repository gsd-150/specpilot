...
type CacheSummary = { kind: "cache_summary"; sequence: number; hit: boolean; stage: EgressStage; request_hash: string; record_hash: string };
...
export type RunEvent = StateTransition | PlanSummary | AgentStep | ToolFinished | CandidateSummary | EvidenceSummary | EgressSummary | CacheSummary | UsageSummary | AnswerOutcome | VerifierSummary | CheckpointSummary | ComplianceSummary | SemanticSummary | RecoverySummary | ResumeSummary | Terminal;
...
export function decodeRunEvent(value: unknown): RunEvent {
  ...
  switch (kind) {
    ...
    case "cache_summary": { const r = exact(value, ["kind", "sequence", "hit", "stage", "request_hash", "record_hash"]); return { kind, sequence, hit: bool(r.hit, "cache hit"), stage: enumeration(r.stage, STAGES, "stage"), request_hash: hash(r.request_hash, "request hash"), record_hash: hash(r.record_hash, "record hash") }; }
