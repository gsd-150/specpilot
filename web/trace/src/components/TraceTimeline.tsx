...
const ERROR_CODES = new Set([
  ...
  "provider_timeout", "provider_unauthorized", "provider_unavailable",
  "provider_unreachable", "provider_cancelled", "provider_unclassified_error",
  "no_adapter_for_route", "transport_replay_refused", "queue_delivery_failed",
  "authorization_clock_invalid", "claim_count_exceeded", "claim_payload_mismatch",
  "claim_unique_excerpts_exceeded", "claim_unique_tokens_exceeded",
  "claim_unique_bytes_exceeded", "corpus_document_cap_missing",
  "corpus_document_unique_excerpts_exceeded", "corpus_document_unique_tokens_exceeded",
  "corpus_document_unique_bytes_exceeded", "corpus_manifest_mismatch",
  "corpus_usage_mismatch", "corpus_unique_excerpts_exceeded",
  "corpus_unique_tokens_exceeded", "corpus_unique_bytes_exceeded",
  "disclosure_fact_mismatch", "document_id_mismatch", "document_version_mismatch",
  "evaluation_root_mismatch", "excerpt_bytes_exceeded", "excerpt_tokens_exceeded",
  "judge_unique_excerpts_exceeded", "judge_unique_tokens_exceeded",
  "judge_unique_bytes_exceeded", "judge_transmitted_tokens_exceeded",
  "judge_transmitted_bytes_exceeded", "online_unique_excerpts_exceeded",
  "online_unique_tokens_exceeded", "online_unique_bytes_exceeded",
  "online_transmitted_tokens_exceeded", "online_transmitted_bytes_exceeded",
  "payload_version_mismatch", "policy_snapshot_mismatch",
  "projected_text_tokens_exceeded", "reservation_accounting_mismatch",
  "reservation_primitive_invalid", "root_unique_excerpts_exceeded",
  "root_unique_tokens_exceeded", "root_unique_bytes_exceeded",
  "root_transmitted_tokens_exceeded", "root_transmitted_bytes_exceeded",
  "route_unauthorized", "source_manifest_mismatch", "source_manifest_unresolvable",
...
function stableError(value: unknown): string | null {
  const valueText = matched(value, STABLE_CODE);
  return valueText !== null && ERROR_CODES.has(valueText) ? valueText : null;
}
...
function project(event: unknown): SafeEvent | null {
  ...
  switch (kind) {
    ...
    case "plan_summary":
      return { sequence: seq, title: "Tool plan", rows: [...row("Steps", integer(safe.step_count)), ...row("Maximum calls", integer(safe.max_tool_calls))] };
    case "agent_step":
      return { sequence: seq, title: "Agent step", rows: [...row("Agent", matched(safe.agent, STABLE_CODE)), ...row("Phase", choice(safe.phase, new Set(["started", "finished"]))), ...row("Duration", integer(safe.duration_ms) === null ? null : `${integer(safe.duration_ms)} ms`), ...row("Stable error", stableError(safe.error_code))] };
    case "tool_finished": { const keys = Array.isArray(safe.argument_keys) ? safe.argument_keys.map((key) => choice(key, ARGUMENT_KEYS)).filter((key): key is string => key !== null).slice(0, 16) : []; const retries = integer(safe.retry_count); const count = integer(safe.result_count); const duration = integer(safe.duration_ms); return { sequence: seq, title: "Tool finished", rows: [...row("Tool", choice(safe.tool, TOOLS)), ...row("Argument keys", keys.length === 0 ? null : keys.join(", ")), ...row("Results", count === null ? null : `${count} results`), ...row("Duration", duration === null ? null : `${duration} ms`), ...row("Retries", retries === null ? null : `${retries} ${retries === "1" ? "retry" : "retries"}`), ...row("Stable error", stableError(safe.error_code))] }; }
