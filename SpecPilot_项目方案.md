...
> **[已实测｜2026-08-15 L2-adv 冻结前核对]** 16 组（dev 6 + locked 10）已全部完成人工 source review 并生成注册 status。冻结前记录以下五条，W6 报告必须逐条披露：
>
> 1. **维度分布偏斜**：实测分布为 normative_strength ×7、document_attribution ×5、role_attribution ×2、request_vs_response ×1、received_vs_generated ×1。成因不是取舍问题：role 与 direction 两维在冻结语料中找不到“主题贴近但角色/方向不同”的真缝隙（§5.2 的排查同样未发现），硬凑只会换出更弱的组。作者决定不换（2026-08-15）：偏斜本身是关于语料的事实发现，如实写清楚比人为凑均匀更有说服力。
> 2. **normative 六组的直喂负例鉴别力弱**：其干扰条款自身就是 SHOULD / MAY / ought-to 级别，被直喂的证据近乎直接反驳 claim，合格 Verifier 必拒。因此直喂漏拦截率接近 0 **不能**被解读为“Verifier 强”，只能解读为该子类构造难度低；该维度的有效信号在端到端负例（语料中存在诱人的 MUST 条款）。
> 3. **正负例非最小改写是该轴的结构必然**：SHOULD 级条款推不出 violating，正例必须落到另一条 MUST 上，因此“同主题但最小改写”在 normative_strength 轴上实际表现为“同一节、不同场景、强度翻转”。§8.1.1 的“最小改写”表述应照此理解。
> 4. **文档归属采用严格文档读法**：document_attribution 四组的负例断言“违反 RFC 9112”，按“违反 9112 文本中的某条要求”判定。若把 RFC 编号当协议套件名读（9112 §1 将 HTTP/1.1 协议定义为三份文档的并集），宽松读法会使四组负例同时失效——该读法不在判定范围内，报告须明说这一前提。
> 5. **locked 集内跨组共享支持条款**：9110 §7.8 的 101 响应条款被三组共享（locked-003 与 locked-004 的正例支持、locked-009 的干扰与支持），9110 §7.6.2 的 MUST 条款被两组共享（locked-002 的正例支持、locked-008 的干扰与支持）。注册闸门只强制 dev 与 locked 互斥、不查 locked 集内共享；合法，但须在报告中披露证据条款的重用。
